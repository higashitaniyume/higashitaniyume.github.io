...
| :--- | :--- | :--- | :--- | :--- |
| **Java 版** (>= 1.21.1) | 普通线路 | `classicbyte.asia:10056` | - | |
| **Java 版** (>= 1.21.1) | 🚀 加速线路 | `ea70d80d.peiqianyun.xyz:54255` | - | |
| **基岩版** (最新版) | 普通线路 | `classicbyte.asia` | 10165 | |
| **基岩版** (最新版) | 🚀 加速线路 | `d3a6ab92.peiqianyun.xyz` | 54250 | |


## 1. 服务器基础环境
本服务器运行在 **Debian 12 (bookworm) x86_64** 操作系统上，部署于 [赔钱云](https://www.peiqianyun.com/clientarea) 的 NAT 实例。

### 1.1 硬件规格
*   **CPU**: Intel Xeon Platinum 8272CL (3核) @ 2.593GHz
*   **内存**: 10GB (物理上限，分配方案：主进程 8G + 基岩转发 2G)
...
| **Java版主进程** | `160.202.254.36:10056` | 25565 | TCP | Java 版玩家连接地址 |
| **基岩互通 (Bed)** | `160.202.254.36:10165` | 19132 | **UDP** | 基岩版 (手机/Win10) 接入 |
| **皮肤上传 API** | `160.202.254.36:10161` | 54259 | TCP | C# 编写的皮肤上传接口进程 |
| **皮肤直链获取** | `160.202.254.36:10159` | 54258 | TCP | Nginx 提供的直链访问及前端页面 |
...
*   `234mc`: 运行 Minecraft Java 版主进程。
*   `skinapi`: 运行 C# 编写的皮肤上传接口服务。
*   `bed`: 运行 Geyser-Standalone 基岩版转发服务。
...
*   **加载器**: Fabric (已加载性能优化、皮肤加载、基岩互通插件共 10 个)。
...
## 4. 基岩互通服务维护 (`bed`)
### 4.1 运行机制
该服务独立运行 **Geyser-Standalone**。它作为一个中间网关，监听内部 UDP `19132` 端口，并将流量转发至主服务器的 `127.0.0.1:25565`。
...
该系统是一个闭环：玩家通过网页上传 -> C# API 保存并通知游戏 -> Nginx 提供 HTTP 直链。
...
| 地址 | 备注 |
| :--- | :--- |
| ea70d80d.peiqianyun.xyz:54255 |Java版MC地址|
| d3a6ab92.peiqianyun.xyz:54250 |基岩版MC地址|
